...
while (F > F_target)
    ...
    % TRAIN CASCADE LEVEL i - strong classifier %
    ...
    disp(i);
   	while (f > fmax)
		...
		% BUILD STRONG CLASSIFIER - ADD NEW SVM %
        ...
        % EVALUATE POS&NEG with strong classifier --> TPR / FPR %
        ...
        for j=1:k
            ...
            final = str2double(fscanf(f_read, '%s', 1));
            if (final == 999999)
                disp ('FINAL STAGE');
        ...
        % GET THRESHOLD %
        th = 0.5*alpha; % Classification Threshold initialization
        TPR = 0; 
        while (TPR < dmin)
            tp =0; fn = 0; fp = 0; tn = 0;
            ...
            for j=1:length(count)
                ...
                elseif (G(j,1) == 0 && count(j,1) == 1) fp = fp+1;
                    % PREPARE POSSIBLE NEGATIVES FOR NEXT STAGE%
    ...
    % 999999 is the marker for end of cascade level.
...
fclose(f_out);
fprintf(file, '%s', strcat('Time consumption: ', num2str(cputime)));
fclose(file);